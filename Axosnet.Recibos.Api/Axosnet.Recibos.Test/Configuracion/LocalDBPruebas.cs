...
{
    [TestClass]
    public class LocalDBPruebas
    {
        private static readonly string _dbName = "RecibosPruebas";
        ...
        public static RecibosContext GetDbContextLocalDb(bool beginTransaction = true)
        {
            var options = new DbContextOptionsBuilder<RecibosContext>()
                .UseSqlServer($"Data Source=(localdb)\\mssqllocaldb;Initial Catalog={_dbName};Integrated Security=True")
                .Options;
            var context = new RecibosContext(options);
            if (beginTransaction)
            {
        ...
        static string Filename => Path.Combine(
           Path.GetDirectoryName(
               typeof(LocalDBPruebas).GetTypeInfo().Assembly.Location),
           $"{_dbName}.mdf");
